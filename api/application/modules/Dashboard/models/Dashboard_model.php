...
class Dashboard_model extends CI_Model
{
	...
	public function getLearnerDashboard($UserId)
	{
		...
		/*################ GET  TOTAL INSTRUCTOR START ##############*/
		$this->db->select('count(u.UserId) as totalinstructor');
		$this->db->join('tblinstructorfollowers tif','tif.InstructorUserId = u.UserId','left');
		$this->db->from('tbluser u');
		$this->db->where('u.RoleId',3);
		$totalinstructor = $this->db->get();
		$allLearnerData['totalinstructor'] = $totalinstructor->result()[0]->totalinstructor;
		/*################ GET  TOTAL INSTRUCTOR END ##############*/
