...
class Category_model extends CI_Model
{
	...
	{	try{
		if($post_Category)
		{
			if(isset($post_Category['ParentId']) && !empty($post_Category['ParentId'])){
			$ParentId = $post_Category['ParentId'];
		}	else {
			$ParentId = '0';
		}
		if($post_Category['IsActive']==1)
			...
			$Category_data = array(
				...
				$res=$this->db->query('call addCategory(?,?,?,?,?,?,?,?,?)',$Category_data);
	...
	public function get_Categorydata($Category_id=Null)
	{
	  ...
	  if($Category_id)
	  {
		...
		$result=$this->db->query('call getCategoryById(?)',$Category_id);
		 $db_error = $this->db->error();
				if (!empty($db_error) && !empty($db_error['code'])) { 
					throw new Exception('Database error! Error Code [' . $db_error['code'] . '] Error: ' . $db_error['message']);
					return false; // unreachable return statement !!!
...
try{
		...
		// $result = $this->db->get('tblmstcategory cp');
		$result=$this->db->query('call getCategories()');
		$db_error = $this->db->error();
				if (!empty($db_error) && !empty($db_error['code'])) { 
					throw new Exception('Database error! Error Code [' . $db_error['code'] . '] Error: ' . $db_error['message']);
					return false; // unreachable return statement !!!
