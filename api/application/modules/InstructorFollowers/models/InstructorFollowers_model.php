...
class InstructorFollowers_model extends CI_Model
{
	...
	try{
		if($InstructorId)
		{
		$result = $this->db->query("SELECT u.UserId,CONCAT(u.FirstName,' ',u.LastName) as Name,u.Title,u.EmailAddress,u.PhoneNumber FROM tbluser u, tblinstructorfollowers i
	...
	try{
		if($post_data){
			$this->db->select('FIND_IN_SET('.$post_data['LearnerId'].',tif.FollowerUserId) as flag,u.UserId,u.FirstName,u.LastName,u.ProfileImage,tif.FollowerUserId');
